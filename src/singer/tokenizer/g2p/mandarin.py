...
from singer.tokenizer.g2p.chinese_model_g2p import BertPolyPredict
from singer.tokenizer.utils.front_utils import generate_poly_lexicon
...
_G2PW_PREDICTORS: Dict[str, BertPolyPredict] = {}
_G2PW_LOCK = threading.Lock()


def _normalize_device(device: Optional[Union[str, int]]) -> str:
    if device is None:
        env_value = os.getenv("F5TTS_G2PW_DEVICE")
        if env_value:
            return _normalize_device(env_value)
        return "auto"
    ...
    if device_str.startswith("cuda:"):
        suffix = device_str.split(":", 1)[1]
        if suffix.isdigit():
            return f"cuda:{suffix}"
        raise ValueError(f"Invalid CUDA device specification: {device}")
    if device_str == "cuda":
        return "cuda:0"
    raise ValueError(f"Unsupported device specification for g2pw: {device}")


def _provider_config_for(device_key: str) -> Tuple[Optional[List[str]], Dict[str, Dict[str, Union[str, int]]]]:
    if device_key == "auto":
        return None, {}
    if device_key == "cpu":
        return ["CPUExecutionProvider"], {}
    if device_key.startswith("cuda:"):
        device_idx = int(device_key.split(":", 1)[1])
        return ["CUDAExecutionProvider", "CPUExecutionProvider"], {"CUDAExecutionProvider": {"device_id": device_idx}}
    return None, {}


def get_g2pw_poly_predict(device: Optional[Union[str, int]] = None) -> BertPolyPredict:
    device_key = _normalize_device(device)
    with _G2PW_LOCK:
        predictor = _G2PW_PREDICTORS.get(device_key)
        if predictor is None:
            providers, provider_options = _provider_config_for(device_key)
            predictor = BertPolyPredict(
            ...
            _G2PW_PREDICTORS[device_key] = predictor
    return predictor
...
word_pinyin_dict = {}
with open(f"{resource_path}/sources/chinese_lexicon.txt", "r", encoding="utf-8") as fread:
...
with open(f"{resource_path}/sources/pinyin_2_bpmf.txt", "r", encoding="utf-8") as fread:
    txt_list = fread.readlines()
    for txt in txt_list:
...
with open(f"{resource_path}/sources/bpmf_2_pinyin.txt", "r", encoding="utf-8") as fread:
    txt_list = fread.readlines()
    for txt in txt_list:
        v, k = txt.strip().split("\t")
...
def chinese_to_bopomofo(text_short, sentence, predictor: BertPolyPredict):
    ...
    for word in words:
        ...
        if word in word_pinyin_dict and word not in poly_dict:
            ...
            for i in range(len(word)):
                c = word[i]
                if c in poly_dict:
                    poly_pinyin = predictor.predict_process([text_short, char_index + i])[0]
                    py = poly_pinyin[2:-1]
                    bopomofos.append(pinyin_2_bopomofo_dict[py[:-1]] + tone_dict[py[-1]])
                    if BLANK_LEVEL == 1:
                        bopomofos.append("_")
...
def _chinese_to_ipa(text, sentence, predictor: BertPolyPredict):
    text = number_to_chinese(text.strip())
    text = normalization(text)
    text = chinese_to_bopomofo(text, sentence, predictor)
...
def chinese_to_ipa(text, sentence, text_tokenizer, *, tokenizer=None):
    # phonemes = text_tokenizer(text.strip())
    device = getattr(tokenizer, "g2p_device", None) if tokenizer is not None else None
    predictor = get_g2pw_poly_predict(device)
    if isinstance(text, str):
        return _chinese_to_ipa(text, sentence, predictor)
    else:
        result_ph = []
        for t in text:
            result_ph.append(_chinese_to_ipa(t, sentence, predictor))
        return result_ph
