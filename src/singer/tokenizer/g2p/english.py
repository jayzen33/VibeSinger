...
def fraction_to_words(numerator, denominator):
    ...
    return (
        " " + _inflect.number_to_words(numerator) + " " + _inflect.ordinal(_inflect.number_to_words(denominator)) + " "
    )
...
def english_to_ipa(text, text_tokenizer):
    ...
    if isinstance(text, str):
        # phonemizer returns a string for single input; only then consider appending a trailing boundary
        if isinstance(phonemes, str) and phonemes and phonemes[-1] in "p⁼ʰmftnlkxʃs`ɹaoəɛɪeɑʊŋiuɥwæjː":
            phonemes += "|_"
